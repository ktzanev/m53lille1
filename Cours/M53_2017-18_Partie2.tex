...
\begin{frame}{Structure des isométries affines}
  \begin{proposition}
    Soient $\phi \in\iso(\ens{E})$, ayant des points fixes $\Omega + \vv{F}$ et $\phi' = T_{\vv{v}}\circ \phi$ ou $\phi' = \phi\circ T_{\vv{v}}$ alors :
    \begin{itemize}[<+(1)->]
      \item si $\vv{v} \in \vv{F}^{\perp}$, $\phi'$  est de «même nature» que $\phi$ et ces points fixes sont de la forme $\Omega' + \vv{F}$;
      \item si $\vv{v} \not\in \vv{F}^{\perp}$, $\phi'$  est une version «glissé» de $\phi$ et n'a pas de points fixes.
    \end{itemize}
  \end{proposition}
\end{frame}
