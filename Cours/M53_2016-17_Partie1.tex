...
\end{frame}
% =======================================
\section{« Rappelles » sur la géométrie plane}
% =======================================
...
\begin{frame}{Le plan réel}
  \begin{itemize}[<+(1)->]
    \item Un ensemble $\ens{P}$ est un \myemph{plan} s'il est isomorphe à $\mathbb{R}^{2}$. Les éléments de $\ens{P}$ sont appelés des \myemph{points} ou des \myemph{vecteurs} en fonction du contexte.
    \item Si on fixe un isomorphisme entre un plan $\ens{P}$ et $\mathbb{R}^{2}$, à tout point (vecteur) $a \in \ens{P}$ on fait correspondre un couple de nombres $(x,y)$ appelé \myemph{coordonnées (cartésiennes)} de $a$.
...
\begin{frame}{Le plan complexe}
  \begin{itemize}[<+(1)->]
    \item Comme $\mathbb{C}$ est isomorphe à $\mathbb{R}^{2}$ on peut le considérer comme un plan (appelé \myemph{le plan complexe}).
