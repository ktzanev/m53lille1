...
\begin{frame}{Définition d'un espace affine euclidien}
  \begin{definition}
    Un ensemble $\ens{E}$ est \alert{metrique} s'il est muni d'un application \alert{distance}\forsimple{\vspace*{-.7\baselineskip}}
    \begin{gather*}
...
\begin{frame}{Distance entre parties}
  ...
  \begin{enumerate}[<+(1)->]
    ...
    \item La propriété précedente reste vrai pour $\ens{A}$ et $\ens{B}$ des sous-espaces affines. De plus $\vv{MN}\perp(\vv{A}+\vv{B})$.
...
\begin{frame}{Rappels : isométrie vectorielle }
  \begin{defprop}
    ...
    \end{enumerate}
  \end{defprop}\pause
  Si une isométrie $\vv{\phi}$ de $\ev{E}$ présèrve un s.e.v. $\ev{F}$ \pause(c.-à-d. $\vv{\phi}(\ev{F})\subset\ev{F}$ $\Leftrightarrow$ $\vv{\phi}(\ev{F})=\ev{F}$)\pause, alors elle présèrve aussi son orthogonal\pause, $\vv{\phi}(\ev{F}^{\perp})=\ev{F}^{\perp}$. \pause
...
\begin{frame}{Dimensions 1 et 2}
  \begin{itemize}[<+(1)->]
    ...
    \item $O_{2} = O_{2}^{+} \sqcup O_{2}^{-}$, où
      ...
      \begin{itemize}[<+(1)->]
        \item $\vv{R}_{\alpha}\circ R_{\beta} = R_{\alpha+\beta}$,
        \item $\vv{S}_{\alpha}\circ S_{\beta} = R_{\alpha-\beta}$,
        \item $\vv{S}_{\alpha}\circ R_{\gamma} = S_{\alpha-\gamma}$ et $R_{\gamma}\circ S_{\beta} = S_{\gamma+\beta}$.
      \end{itemize}\pause
...
\begin{frame}{Les isométries de $\mathbb{C}$ (dimension 2)}
  En identifiant l'espace euclidient $\mathbb{R}^{2}$ avec $\mathbb{C}$, le produit scalaire s'ecrit :
  \[
    \scalprod{z}{w} = \frac{\overline{z}w+z\overline{w}}{2}
  \]\pause
  Toute élément de $O(\mathbb{C})$ est de la forme
  \begin{itemize}[<+(1)->]
    \item $\rho_{a} : z \mapsto az$ avec $|a| =1$, et dans ce cas c'est une rotation d'angle $\arg(z)$,
  ...
  \begin{itemize}[<+(1)->]
    ...
    \[
      \vv{R}_{\alpha}=
        ...
        \end{psmallmatrix}.
    \]\pause
    Dans ce cas $\vv{\phi} = \vv{\rho}_{\vv{w},\alpha}$ est la rotation de $\alpha$ autour de l'axe orienté par $\vv{w}$.
    \item $\vv{\phi} \in O^{-}(\ev{E})$ ssi il existe une b.o.n $\{\vv{u},\vv{v},\vv{w}\}$ dans laquelle la matrice de $\vv{\phi}$ est sous-la forme
    \[
      \begin{psmallmatrix}
        \cos(\alpha)&-\sin(\alpha)&0\\
...
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% -----------
\begin{frame}{Forme standard des isométrie}
  \begin{proposition}
    ...
    \end{center}\pause
    Et pour $\vv{\phi} \in O^{-}(\ev{E})$, à la place d'un nombre impaire de $1$ il y a des $-1$.
  \end{proposition}
\end{frame}
...
\begin{frame}{Décomposition des isométries en réflexions}
  Toute symétrie orthogonale par rapport à un sous espace vectoriel est une isométrie\pause, directe si la codimension de cette espace est paire\pause, ou indirecte si la codimension est impaire.\pause
  \begin{definition}
  ...
  \begin{proposition}
    ...
    Si $k$ est paire $\vv{\phi} \in O^{+}(\ev{E})$, si $k$ est impaire $\vv{\phi} \in O^{-}(\ev{E})$.
  \end{proposition}
\end{frame}
...
\begin{frame}{Définition}
  \begin{defprop}
    On dit qu'une application affine $\phi \in\aff(\ens{E})$ est une \alert{isométrie} si une des conditions équivalente est satisfaite.
    \begin{itemize}[<+(1)->]
...
\begin{frame}{Premières propriétés des isométries}
  \begin{itemize}[<+(1)->]
    ...
    \item Toute translation est le produit de deux refléxions.
  \end{itemize}
\end{frame}
...
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% -----------
\begin{frame}{Structure des isométrie affines}
  \begin{lemma}
  ...
  \begin{proposition}
    ...
    \begin{itemize}[<+(1)->]
      ...
      \item soit il existe un unique $\vv{v} (\neq 0)$, vecteur fixe de $\vv{\phi}$, tel que $T_{\vv{v}}\circ\phi = \phi\circ T_{\vv{v}}$ possède un point fixe.
    \end{itemize}
  \end{proposition}
\end{frame}
...
\begin{frame}{Dimensions 1 et 2}
  \begin{itemize}[<+(1)->]
    ...
    \myemph{($\phi$ est la composé d'au plus $2$ refléxions.)}
    \item $\iso(\mathbb{R}^{2}) = \iso^{+}(\mathbb{R}^{2}) \sqcup \iso^{-}(\mathbb{R}^{2})$.
      \begin{itemize}[<+(1)->]
        \item $\phi\in\iso^{+}(\mathbb{R}^{2})$ ssi
          \begin{itemize}\pause
        ...
        \item $\phi\in\iso^{-}(\mathbb{R}^{2})$ ssi
          ...
          \end{itemize}
      \end{itemize}\pause
    \myemph{($\phi$ est la composé d'au plus $3$ refléxions.)}
  \end{itemize}
\end{frame}

% -----------
\begin{frame}{Les isométrie affines de $\mathbb{C}$}
  \textit{Rappelle :} Les application affines de l'espace euclidient $\mathbb{C}$ sont de la forme $z \mapsto \alpha z+\beta\overline{z}+\gamma$.\pause\medskip
    ...
    \item $\iso(\mathbb{R}^{3}) = \iso^{+}(\mathbb{R}^{3}) \sqcup \iso^{-}(\mathbb{R}^{3})$.
      \begin{itemize}[<+(1)->]
        ...
        \item $\phi\in\iso^{-}(\mathbb{R}^{3})$ ssi
          ...
          \end{itemize}
      \end{itemize}\pause
    \myemph{($\phi$ est la composé d'au plus $4$ refléxions.)}
  \end{itemize}
\end{frame}
...
% -----------
\begin{frame}{Décomposition des isométries en réflexions}
  \myemph{Rappelle : Une réflexion est une isométrie indirecte.}\pause
  \begin{proposition}
    ...
    \]\pause
    Si $k$ est paire $\phi \in \iso^{+}(\ens{E})$, et si $k$ est impaire $\phi \in \iso^{-}(\ens{E})$.
  \end{proposition}
\end{frame}
