...
\begin{frame}{Définition d'une similitude}
  \begin{definition}
    \begin{itemize}[<+(1)->]
      \item Une application linéaire $\vv{\phi} \in \lin(\ev{E})$ est dit \alert{similitude vectorielle} si elle multiplie les normes par une constante $k > 0$ :
      \[
          \norm{\vv{\phi}(\vv{v})} = k\norm{\vv{v}},\qquad \forall \vv{v} \in \ev{E}.
      \]
      \item Une application affine $\phi  \in \aff(E)$ est dit \alert{similitude affine} si elle multiplie les distance par une constante $k > 0$ :
      \[
...
\begin{frame}{Propriétés des similitudes}
  \begin{itemize}[<+(1)->]
    ...
    \item Les similitudes vectoriels (resp. affines, resp. directes) forment un groupe.
