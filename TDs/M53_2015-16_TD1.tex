...
\begin{exo} (Droites)
  ...
  \begin{enumerate}
    \item\label{q:reel} On consid\`ere l'espace affine $\mathbb{R}^{2}$. Soit $\mathcal{D}$ la droite $\big\{(x,y) \;\big|\; x+2y=1\big\}$.
    \begin{enumerate}
      ...
      \item Déterminer les équations linéaires qui définissent la droite parallèle à $\mathcal{D}$ qui passe par $A=(2,1)$.
...
\begin{exo} (Exemples d'espaces affines)
  \begin{enumerate}
    \item Montrer que pour tout $n \in \N^\ast$ l'ensemble $\ens{E}_{n}$ des polynômes unitaires de degré $n$ à coefficients réels est un espace affine. Quelle est sa dimension? En donner un repère affine.
    \item Soient $d$ un entier non nul, et $a_1,\ldots, a_d$ et $b$ des réels. Soit $\ens{E}$ l'ensemble des suites réelles $u=\big(u_n\big)_{n\in\N}$ vérifiant
      \[
        u_{n+d} = a_{1} u_{n+d-1} + \cdots + a_{d-1} u_{n+1} + a_d u_n + b \quad \text{pour tout }n \in \N.
      \]
      \begin{enumerate}
        \item Montrer que $\ens{E}$ est un espace affine.
        \item Quelle est sa dimension?
        \item Décrire les éléments de $\ens{E}$ lorsque $d=1$ (distinguer les cas $a_1 \neq 1$ et $a_1=1$).
        \item On suppose maintenant $d=2$. Donner une base de la direction $\vv{\ens{E}}$ de $\ens{E}$. Donner un repère affine de $\ens{E}$ dans le cas où $a_1+a_2 \neq 1$.
      \end{enumerate}
    \end{enumerate}
\end{exo}
...
\begin{exo} (Changement de repères)
  ...
  \begin{enumerate}
    ...
    \item\hard On note $\mathfrak{Rep}$ l'ensemble des repères de $\mathbb{R}^{3}$. Montrer que l'application:
      \[
        Act:
        \begin{cases}
...
\begin{exo} (Homothéties)
  ...
  \begin{enumerate}
    ...
    \item Montrer que les homoth\'eties et les translations forment un sous-groupe $HT_2(\mathbb{R})$ du groupe affine, puis que ce sous-groupe est engendré par les homothéties. Le groupe $HT_2(\mathbb{R})$ agit-il transitivement sur le plan affine?
